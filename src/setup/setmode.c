...
int GrSetGraphicsMode(int width, int height, int bpp, int noclear)
{
    GrGraphicsMode gm = GR_width_height_bpp_graphics;

    if (noclear) gm = GR_NC_width_height_bpp_graphics;

    return GrSetMode(gm, width, height, bpp);
}

int GrUnsetGraphicsMode(void)
{
...
int GrSetMode(GrGraphicsMode which,...)
{
    ...
    if (c) for (pl = 1; (pl < 32) && ((1UL << pl) < c); pl++);
