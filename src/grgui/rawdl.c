...
typedef struct _guiRawDataList {
    ...
    int vreg;             // max num of complete regs visible (gen)
    int partial;          // 1 if the vreg reg is partialy visible (gen)
                              ...
                              GrColor bg, GrColor fg, void **reg, int nreg, int sreg)
{
    ...
    if (rdl->vreg >= rdl->nreg) {
        ...
        rdl->eheight = rdl->height;
        rdl->partial = (((rdl->height - 6) % rdl->lineheight) > 3) ? 1 : 0;
    }
...
static void show_rdl_item(GUIRawDataList *rdl, int ind)
{
    ...
    y2 = y + rdl->lineheight - 1;
    // The last record in screen probably does not fit complete
    yl = rdl->y + rdl->dy + rdl->eheight - 4;
    if (y2 > yl) y2 = yl;
    if (y2 < y) return; 

    if (ind+rdl->freg == rdl->sreg) selected = 1;

    GrFilledBox(x, y, x+rdl->ewidth-7, y2, selected ? rdl->fg : rdl->bg);

    if (ind+rdl->freg >= rdl->nreg) return;

    GrSetClipBox(x+1, y, x+rdl->ewidth-8, y2);
            ...
            switch (c) {
                case GRMOUSE_LB_PRESSED :
                    if (GrCheckCoordInto(x, y, rdl->x, rdl->y,
                        ...
                        if ((pos >= 0) && (pos < rdl->vreg+rdl->partial) &&
                            (pos+rdl->freg < rdl->nreg)) {
                            if (ind != pos) {
                                old = ind;
                                ind = pos;
                                ...
                                change = 1;
                                //if (check_freg(rdl) != 0)
                                //    move = 1; // occurs when pos == rdl->vreg
                            }
                        res = 1;
                        }
                    } else if (rdl->vscb) {
